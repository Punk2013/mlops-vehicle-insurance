...
Различные версии моделей сохранятся в `models/`. В cache сохранаяется добавленные из потока батчи данных `train_data`, данные о позиции в потоке `rows_read.txt`, данные о качестве данных `data_quality.pkl`, выводимые через режим `summary`(см. ниже) и обученное на добавленных батчах преобразование `transformer.pkl`, которое применяется после базовой очистки данных.
